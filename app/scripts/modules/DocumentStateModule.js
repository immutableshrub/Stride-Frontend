...
class DocumentStateModule {
  ...
  constructor(documentPotentialState) {
    ...
    log.debug(
      ...
      "Document State Manager is initalising...",
      uiColors.pink
    );
    ...
    opsHandlers.register("all", () => {
      ...
      docGenerate.forEach((doc, id) => {
        window.dispatchEvent(
          new CustomEvent("CanvasDisplayEvent-AddElement", {
            detail: {
              data: {
                id: id,
                ...doc.data,
    ...
    opsHandlers.register("pch", (e) => {
      window.dispatchEvent(
        new CustomEvent("SharedStateRelay-DSMG-ioComm", {
          detail: {
    ...
    window.addEventListener(
      "DocumentStateModule-DSMG-ioComm",
      function (e) {
        //console.log(e)
    ...
    window.addEventListener(
      "DocumentStateEvent-DocumentUndo",
      function (e) {
        slfFunctions.undo();
      },
      false
    ...
    window.addEventListener(
      "DocumentStateEvent-DocumentRedo",
      function (e) {
        slfFunctions.redo();
      },
      false
    ...
    window.addEventListener(
      "DocumentStateEvent-DocumentClearAll",
      function (e) {},
      false
    );
    ...
    window.addEventListener("DocumentStateEvent-File-Export", function (e) {
      ...
      const dataHash = new TextEncoder().encode(
        SHA3(rawData, { outputLength: 512 })
      );
      const generationDate = Uint8Array.from(
      ...
      const fdownloadURL = URL.createObjectURL(
        new File([dv], fname, {
    ...
    document.querySelector("title").innerText =
      this.documentState.name + " - Playspace Whiteboard";
  }
}
