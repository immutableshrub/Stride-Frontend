...
function registerMenuBarItems() {
  WindowState.ui.menubar.forEach((tool, index) => {
    ...
    if (tool.type == "spacer") {
      ...
      uiDocument.components.menubar.addItem(
        tool.id || "tool" + index,
        tool.name,
        uiIcons[tool.icon.split("/")[0]][tool.icon.split("/")[1]],
        tool.type,
        toolAction,
        tool.active || false
      );
...
function loader(num, text) {
  document
    .querySelector('main.loader[uie-ref="LoaderWrapper"]')
    .setAttribute("data-state", num || 0);
  document
    .querySelector('main.loader[uie-ref="LoaderWrapper"]')
    .setAttribute("data-loadingText", text || "");
}
