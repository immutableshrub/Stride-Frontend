...
var Color = class {
  ...
  cmyk() {
    ...
    const m = (1 - g - k) / (1 - k);
    const y5 = (1 - b - k) / (1 - k);
    const color = new Color(c, m, y5, k, "cmyk");
    return color;
  }
  hsl() {
    const { _a, _b, _c } = this.rgb();
  ...
  xyz() {
    ...
    const color = new Color(x5, y5, z, "xyz");
    return color;
  }
  _clamped() {
    const { _a, _b, _c } = this.rgb();
...
function point(x5, y5) {
  return new Point(x5, y5).transform(this.screenCTM().inverse());
}
...
var Box = class {
  ...
  merge(box) {
    ...
    return new Box(x5, y5, width4, height4);
  }
  toArray() {
...
function dispatch(node, event, data2, options) {
  ...
  if (event instanceof globals.window.Event) {
    ...
    event = new globals.window.CustomEvent(event, __spreadValues({ detail: data2, cancelable: true }, options));
    n.dispatchEvent(event);
  }
  return event;
}
...
extend(Runner, {
  ...
  transform(transforms2, relative, affine) {
    ...
    function run(pos) {
      if (!relative)
        this.clearTransform();
      const { x: x5, y: y5 } = new Point(origin).transform(element._currentTransform(this));
